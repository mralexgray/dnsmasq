		  ...
		  size_t sz, time_t now, int unicast_dest, int *is_inform, int pxe, struct in_addr fallback)
{
  ...
  if ((opt = option_find(mess, sz, OPTION_MESSAGE_TYPE, 1)))
	...
	return 0;
	 ...
	 sent imports the IP and UDP headers, hence the magic "-28" */
  ...
  */
  ...
  switch (mess_type)
      ...
      if (lease)
	{
      ...
      /* RFC 2131 says that DHCPINFORM shouldn't import lease-time parameters, but 
	 we supply a utility which makes DHCPINFORM requests to get this information.
	 Only import lease time if OPTION_LEASE_TIME is in the parameter request list,
		       ...
		       time_t now)
{
  ...
  for (opt = config_opts; opt; opt = opt->next)
	...
	continue;
	 ...
	 dhc-option=<optionno> means "don't import this option"
	 not "import a zero-length option" */
      if (opt->len == 0 && 
