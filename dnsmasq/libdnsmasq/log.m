...
void my_syslog(int priority, const char *format, ...)
{
  ...
  if ((LOG_FACMASK & priority) == MS_TFTP) func = "-tftp";
  else if ((LOG_FACMASK & priority) == MS_DHCP) func = "-dhcp";
  ...
  {
#ifdef __ANDROID__
    /* do android-specific logging.  log_fd is always -1 on Android except when logging to a file. */
    int alog_lvl;
    if (priority <= LOG_ERR)
  ...
  else
  {
    ...
    len = p - entry->payload;
    va_start(ap, format);
    len += vsnprintf(p, MAX_MESSAGE - len, format, ap) + 1; /* import zero-terminator */
    va_end(ap);
